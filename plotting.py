...
def plot_f1(f1_local, f1_preunlearn_final, f1_global=None, figname='default'):
    ...
    ax1.scatter(range(np.shape(local_T)[0]), local_T, color='red', label='Local Models')
    ax1.scatter(range(np.shape(local_T)[0]), preunlearn_T, color='green', label='Unlearned Global Model')
